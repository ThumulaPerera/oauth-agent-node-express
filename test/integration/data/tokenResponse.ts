import * as jose from 'jose';

// Only used for testing
const pkcs8 = `-----BEGIN PRIVATE KEY-----

ak6k+8+UFd93M4z0Qo74OhXB
-----END PRIVATE KEY-----`

async function createJwt(payload: any, issuer: string, audience: string | string[]): Promise<string> {
    const alg = 'RS256'

    const privateKey = await jose.importPKCS8(pkcs8, alg)

    const jwt = await new jose.SignJWT(payload)
        .setProtectedHeader({ alg })
        .setIssuedAt()
        .setIssuer(issuer)
        .setAudience(audience)
        .setExpirationTime('2h')
        .sign(privateKey)

    return jwt;
}

const samplePayload = {
    sub: "user1@example.com",
}

async function generateTokenResponse(issuer: string, audience: string | string[], payload: any = samplePayload)
    : Promise<any> {
    return {
        access_token: "54bd024f-5080-3db5-9422-785f5d610605",
        refresh_token: "e2a6e9d4-9d2f-3e2c-9c0c-7b9f2c7f5a9e",
        scope: "openid",
        id_token: await createJwt(payload, issuer, audience),
        token_type: "Bearer",
        expires_in: 3600
    }
}

export { generateTokenResponse }
